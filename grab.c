/* TODO
   - nikoli championship puzzles: if multiple puzzles have same the vol and
     puzzle type, only the first one encountered will be grabbed
   - janko akari 530 has wildly differing format, grab it manually...
   - some random puzzles throughout on janko that won't load, but they are
     mostly retracted duplicates, so don't care about them
*/
...
int parsedifficulty(char *s) {
	...
	if(!strcmp(s,"schwer")) return 9;
	return -1;
}

char *findnextdifftag(char *p) {
	const char *findtag="<th>";
	const char *findtag2="<td>";
	const char *findtag3="<td  style=\"text-align: center\">";
	char *q,*r,*s;
	q=strstr(p,findtag);
	r=strstr(p,findtag2);
	s=strstr(p,findtag3);
	if(!q && !r && !s) return 0;
	if(q && (q<r || !r) && (q<s || !s)) return q+strlen(findtag);
	else if(r && (r<q || !q) && (r<s || !s)) return r+strlen(findtag2);
	else if(s && (s<q || !q) && (s<r || !r)) return s+strlen(findtag3);
	else { puts("something strange happened in diff finding"); return 0; }
}

void processjanko(char *u,char *v) {
	const char *findpuzzle="<title>",*findnum="href=\"",*index="index.htm";
	const char *finddifficult=">Sortiert nach Schw";
	...
	while(1) {
		...
		if(curdiff<0) printf("unknown difficulty: %s\n",token(p));
		while(1) {
			q=findnextdifftag(p);
			r=strstr(p,findlevelno);
			if(q && parsedifficulty(token(q))<0) q=r;
			if(!q || !r) goto nodiff;
			if(q<r) break;
			r+=strlen(findlevelno);
			if(!isdigit(*r)) goto nodiff;
			tall=parsenum(r);
			diff[tall]=curdiff;
			p=r;
		}
...
void processhackercoil(char *name,char *pw) {
	...
	for(lev=0;;lev++) {
		...
		/* extremely arbitrary difficulty setting */
		if(lev<2) dif=0;
		else if(lev<3) dif=1;
		else if(lev<7) dif=2;
